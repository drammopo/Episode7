...
puts "---------Benchmarking 2 city route---------"
phil = SalesPerson.new
all_cities.shuffle.take(2).each do |city|
  phil.schedule_city(Place.build("#{city}, TX"))
end
...
puts "\n\n---------Benchmarking 10 city route---------"
bob = SalesPerson.new
all_cities.shuffle.take(10).each do |city|
  bob.schedule_city(Place.build("#{city}, TX"))
end
...
puts "\n\n---------Benchmarking 50 city route---------"
ted = SalesPerson.new
all_cities.shuffle.take(50).each do |city|
  bob.schedule_city(Place.build("#{city}, TX"))
end
...
puts "\n\n---------Benchmarking 200 city route---------"
chuck = SalesPerson.new
all_cities.shuffle.take(200).each do |city|
  chuck.schedule_city(Place.build("#{city}, TX"))
end
